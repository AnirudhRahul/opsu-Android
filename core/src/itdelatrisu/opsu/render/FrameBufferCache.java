...
public class FrameBufferCache {
	...
	private ArrayMap<HitObject, Rendertarget> cacheMap;

	/** */
	private Array<Rendertarget> cache;
	...
	private FrameBufferCache() {
		cache = new Array<Rendertarget>();
		cacheMap = new ArrayMap<HitObject, Rendertarget>();
	}
	...
	public boolean freeMappingFor(HitObject obj) {
		return cacheMap.removeKey(obj) != null;
	}
	...
	public Rendertarget insert(HitObject obj) {
		...
		for (int i = 0; i < cache.size; ++i) {
			buffer = cache.get(i);
			if (!cacheMap.containsValue(buffer,true)) {
