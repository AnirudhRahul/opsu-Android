...
public class DownloadsMenu extends BasicGameState {
	...
	private static final DownloadServer[] SERVERS = {new OsuMirrorServer(),
		new MnetworkServer(),
		new HexideServer(),
		new BloodcatServer(),
		new YaSOnlineServer(),
			...
			try {
				...
				if (!interrupted) {
					...
					if (nodes == null)
						...
						if (query.isEmpty())
							searchResultString = "Type to search!, Double tap to download";
...
//		UI.getNotificationManager().sendNotification("Beatmaps you have downloaded elsewhere won't appear here, but can still be imported", Color.cyan);
...
int bgIndex=-1;
	...
	public void render(GameContainer container, StateBasedGame game, Graphics g)
		...
		// background
		Image[] list={GameImage.SEARCH_BG1.getImage(),GameImage.SEARCH_BG2.getImage(),GameImage.SEARCH_BG3.getImage()};
		Image bg = list[bgIndex];
